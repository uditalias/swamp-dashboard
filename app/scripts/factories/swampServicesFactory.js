...
angular.module('swamp.services').factory('swampServicesFactory', [
  '$rootScope', 'env', 'SERVICE_STATE', 'CLIENT_REQUEST', 'aggregatedDataFactory', 'AGGREGATED_LIST_TYPE', 'LOG_TYPE', 'serializeService', 'EVENTS', 'swampManager', 'modalService', 'MODAL_TYPE', 'speechService', 'SPEAKABLE_TEXT',
  function ($rootScope, env, SERVICE_STATE, CLIENT_REQUEST, aggregatedDataFactory, AGGREGATED_LIST_TYPE, LOG_TYPE, serializeService, EVENTS, swampManager, modalService, MODAL_TYPE, speechService, SPEAKABLE_TEXT) {

    var CPU_WARNING_LEVEL = 90;
    ...
    SwampService.prototype = {
      ...
      updateMonitorData: function (monitorData) {
        ...
        if ((this.monitor.cpu || !isNaN(this.monitor.cpu)) && monitorData.cpu) {
          if (monitorData.cpu > this.monitor.cpu) {
            ...
            if (monitorData.cpu > CPU_WARNING_LEVEL) {
              speechService.speak(
                SPEAKABLE_TEXT.SERVICE_HIGH_CPU_USAGE.text.format(this.name,
                  monitorData.cpu.toFixed(0)), SPEAKABLE_TEXT.SERVICE_HIGH_CPU_USAGE.enqueue
              );
            }
      ...
      getContextMenu: function () {
        ...
        var contextMenu = [{
          title: 'Out log',
          command: function () {
            $rootScope.$broadcast(EVENTS.OPEN_FOOTER_PANEL, this.outLogData.id);
          }.bind(this)
        }, {
          title: 'Error log',
          command: function () {
            $rootScope.$broadcast(EVENTS.OPEN_FOOTER_PANEL, this.errorLogData.id);
          }.bind(this)
        }, {
          title: 'Restart',
          ...
          command: [{
            title: 'STDOUT',
            command: function () {
              this.openIOStreamer('out');
            }.bind(this)
          }, {
            title: 'STDERR',
            command: function () {
              this.openIOStreamer('error');
            }.bind(this)
          }]
        }];
